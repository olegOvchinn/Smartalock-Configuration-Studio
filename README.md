...
As with the Smartalock wizard, module numbers don't match click-through order here either: the actual path is **01 → 02 → 03 → 00 → 04 → 05 → 06 → 07 → 08 → (09 if switched on) → 10 → (11 if switched on) → (12 if switched on) → (13 if switched on)**. `floorsense-reservation-types-01.html` is the true entry page (its back-arrow returns to `pick-a-product.html`), then continues to `floorsense-checkin-methods-02.html`, then to `floorsense-desk-user-groups-03.html`, then to `floorsense-setup-path-00.html`, then on through Prebooking, Business Hours, and the Optional Features hub, then Default Reservation Times and Check-in Window (both always visited), then Auto-Release on Idle, Reservation Length, NSNP, FME, and Anti Desk Hogging — with the four optional pages (09, 11, 12, 13) transparently skipped over during Next/Back click-through whenever their switch on the Optional Features hub is off, via a small `NAV_SEQUENCE`/`seqResolve()` walker duplicated on the five pages adjacent to an optional page. The progress bar still lists all fourteen modules in their fixed 00–13 numeric order regardless of click-through order or toggle state, and every segment stays fully clickable — the same convention used for the Smartalock wizard's 00/02 reorder. Its state lives under `project.floorsense.wizard` in the same `configurationProject` localStorage key described below — `setupPath`, `reservationTypes`, `checkinMethods`, `deskUserGroups`, `prebookingAdvance`, `businessHours`, `defaultReservationTimes`, `checkinWindow`, `idleRelease`, `reservationLength`, `nsnp`, `fme` and `antiDeskHogging` respectively; the Optional Features hub (module 06) introduces no state object of its own — its four switches read and write those same existing `idleRelease.enabled`/`nsnp.mode`/`fme.mode`/`antiDeskHogging.mode` fields directly, only recording its own `moduleStatus` entry for the progress bar. Module 13 is currently the last one built, so its next-arrow is disabled.
