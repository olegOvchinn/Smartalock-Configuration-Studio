...
| 04 | `floorsense-default-reservation-times-04.html` | Default Reservation Times (default booking start, end and duration) |
| 05 | `floorsense-checkin-window-05.html` | Check-in Window (how early/late a user can check in) |
| 06 | `floorsense-idle-release-06.html` | Auto-Release on Idle |
| 07 | `floorsense-prebooking-advance-07.html` | Prebooking — How Far In Advance |
| 08 | `floorsense-reservation-length-08.html` | Default Reservation Length |

As with the Smartalock wizard, module numbers don't match click-through order here either: the actual path is **01 → 02 → 00 → 03 → 04 → 05 → 06 → 07 → 08**. `floorsense-reservation-types-01.html` is the true entry page (its back-arrow returns to `pick-a-product.html`), then continues to `floorsense-checkin-methods-02.html`, then to `floorsense-setup-path-00.html`, then on through Business Hours, Default Reservation Times, Check-in Window, Auto-Release on Idle, Prebooking, and Reservation Length in numeric order. The progress bar still lists all nine modules in their fixed 00–08 numeric order regardless of this click-through order — the same convention used for the Smartalock wizard's 00/02 reorder. Its state lives under `project.floorsense.wizard` in the same `configurationProject` localStorage key described below — `setupPath`, `reservationTypes`, `checkinMethods`, `businessHours`, `defaultReservationTimes`, `checkinWindow`, `idleRelease`, `prebookingAdvance` and `reservationLength` respectively. Module 08 is currently the last one built, so its next-arrow is disabled.
...
Business Hours (module 03) is a single-topic page in the same one-card, one-timeline explainer style as Check-in Window: one card with a live diagram driven by a time drum on either side (Opens / Closes), setting the standard business hours used as a default for most users. Default Reservation Times (module 04) and Check-in Window (module 05) were originally one combined page ("Check-in Policy") hosting two numbered sections; they were later split into their own single-topic pages, each built as a single explainer card: an icon + description, a live diagram, and a footer strip with supporting context. Default Reservation Times comes right after Business Hours so its default start time is already saved by the time a user reaches Check-in Window, whose own worked example depends on it. Auto-Release on Idle (module 06) uses the same card pattern but as its own single-topic page too, since it was originally going to be a third numbered section on the combined page before being split out.
