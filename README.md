...
| 03 | `floorsense-prebooking-advance-03.html` | Prebooking — How Far In Advance |
| 04 | `floorsense-business-hours-04.html` | Business Hours (standard opening/closing time used for most users) |
| 05 | `floorsense-default-reservation-times-05.html` | Default Reservation Times (default booking start, end and duration) |
| 06 | `floorsense-checkin-window-06.html` | Check-in Window (how early/late a user can check in) |
| 07 | `floorsense-idle-release-07.html` | Auto-Release on Idle *(optional; off by default)* |
| 08 | `floorsense-reservation-length-08.html` | Default Reservation Length |
| 09 | `floorsense-nsnp-09.html` | No Swipe No Power — NSNP *(optional; requires NSNP relay units installed at each desk)* |
| 10 | `floorsense-fme-10.html` | Follow Me Ergonomics — FME *(optional; save/apply a custom sit-stand desk height via the FS mobile app)* |
| 11 | `floorsense-anti-desk-hogging-11.html` | Anti Desk Hogging *(optional, off by default; restricts booking the same desk repeatedly within a set time window)* |

As with the Smartalock wizard, module numbers don't match click-through order here either: the actual path is **01 → 02 → 00 → 03 → 04 → 05 → 06 → 07 → 08 → 09 → 10 → 11**. `floorsense-reservation-types-01.html` is the true entry page (its back-arrow returns to `pick-a-product.html`), then continues to `floorsense-checkin-methods-02.html`, then to `floorsense-setup-path-00.html`, then on through Prebooking, Business Hours, Default Reservation Times, Check-in Window, Auto-Release on Idle, Reservation Length, NSNP, FME, and Anti Desk Hogging in numeric order. The progress bar still lists all twelve modules in their fixed 00–11 numeric order regardless of this click-through order — the same convention used for the Smartalock wizard's 00/02 reorder. Its state lives under `project.floorsense.wizard` in the same `configurationProject` localStorage key described below — `setupPath`, `reservationTypes`, `checkinMethods`, `prebookingAdvance`, `businessHours`, `defaultReservationTimes`, `checkinWindow`, `idleRelease`, `reservationLength`, `nsnp`, `fme` and `antiDeskHogging` respectively. Module 11 is currently the last one built, so its next-arrow is disabled.

Module 00 also branches: choosing **Relaxed**, **Pre-Configured**, or **Strict** all route to `floorsense-out-of-box-policy.html` — an unnumbered, read-only summary of Floorsense's default policy that skips the rest of the walkthrough entirely (Relaxed/Strict pass a `?preset=` query string so the page can reflect the choice in its heading, though the underlying numbers are currently shared with Pre-Configured pending distinct values). Choosing **Build Your Own** continues into module 03 (Prebooking), since desk allocation and check-in methods were already collected on modules 01 and 02 before reaching this page.
