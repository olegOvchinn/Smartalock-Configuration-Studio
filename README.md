...
| 03 | `floorsense-default-reservation-times-03.html` | Default Reservation Times (default booking start, end and duration) |
| 04 | `floorsense-checkin-window-04.html` | Check-in Window (how early/late a user can check in) |
| 05 | `floorsense-idle-release-05.html` | Auto-Release on Idle |
| 06 | `floorsense-prebooking-advance-06.html` | Prebooking — How Far In Advance |
| 07 | `floorsense-reservation-length-07.html` | Default Reservation Length |

As with the Smartalock wizard, module numbers don't match click-through order here either: the actual path is **01 → 02 → 00 → 03 → 04 → 05 → 06 → 07**. `floorsense-reservation-types-01.html` is the true entry page (its back-arrow returns to `pick-a-product.html`), then continues to `floorsense-checkin-methods-02.html`, then to `floorsense-setup-path-00.html`, then on through Default Reservation Times, Check-in Window, Auto-Release on Idle, Prebooking, and Reservation Length in numeric order. The progress bar still lists all eight modules in their fixed 00–07 numeric order regardless of this click-through order — the same convention used for the Smartalock wizard's 00/02 reorder. Its state lives under `project.floorsense.wizard` in the same `configurationProject` localStorage key described below — `setupPath`, `reservationTypes`, `checkinMethods`, `defaultReservationTimes`, `checkinWindow`, `idleRelease`, `prebookingAdvance` and `reservationLength` respectively. Module 07 is currently the last one built, so its next-arrow is disabled.

Module 00 also branches: choosing **Relaxed**, **Pre-Configured**, or **Strict** all route to `floorsense-out-of-box-policy.html` — an unnumbered, read-only summary of Floorsense's default policy that skips the rest of the walkthrough entirely (Relaxed/Strict pass a `?preset=` query string so the page can reflect the choice in its heading, though the underlying numbers are currently shared with Pre-Configured pending distinct values). Choosing **Build Your Own** continues into module 03 (Default Reservation Times), since desk allocation and check-in methods were already collected on modules 01 and 02 before reaching this page.

Default Reservation Times (module 03) and Check-in Window (module 04) were originally one combined page ("Check-in Policy") hosting two numbered sections; they were later split into their own single-topic pages, each built as a single explainer card: an icon + description with the actual settings editable inline in the sentence, a shaded "EXAMPLE" panel with a small diagram, and a footer strip with supporting context. Default Reservation Times comes first so its default start time is already saved by the time a user reaches Check-in Window, whose own worked example depends on it. Auto-Release on Idle (module 05) uses the same card pattern but as its own single-topic page too, since it was originally going to be a third numbered section on the combined page before being split out.
