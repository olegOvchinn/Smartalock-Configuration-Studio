...
The Configuration Summary also delivers on that "layer on neighbourhood-specific policies" note: a conditional **Additional Policies for Desk Groups** section, hidden unless `project.floorsense.wizard.deskUserGroups.deskGroups.none` is `false` (i.e. Desk Groups are actually in use). When shown, a "+ Duplicate Core Policy" button lets the client clone the core policy — the same five groups that already carry an Edit pill on this page (Check-in Window, Prebooking & Duration, Check-in Behaviour, Access Methods, and Additional Options/NSNP-sit-stand-recurring-DND) — under a custom name like "Finance" or "CEO," then tweak the clone's own numbers/toggles independently, up to 3 additional policies. Each is a `.policy-card` with real form controls (number inputs, `.policy-switch` on/off toggles ported from the Optional Features hub, a NSNP `<select>`, and checkbox rows for Access Methods) instead of the read-only text used for the core policy above, autosaving on every change like the rest of the wizard. State lives at `project.floorsense.wizard.additionalPolicies` — a new top-level array of up to 3 objects, each independent of the core policy's own (still-hardcoded, non-dynamic) summary text. This is purely additive to the Configuration Summary; it isn't wired into "Confirm & finish," the end-user PDF guide, or any other page's Edit links yet.
