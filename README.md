...
As with the Smartalock wizard, module numbers don't match click-through order here either: the actual path is **01 → 02 → 00 → 03 → 04 → 05 → 06 → 07 → 08 → 09**. `floorsense-reservation-types-01.html` is the true entry page (its back-arrow returns to `pick-a-product.html`), then continues to `floorsense-checkin-methods-02.html`, then to `floorsense-setup-path-00.html`, then on through Prebooking, Business Hours, Default Reservation Times, Check-in Window, Auto-Release on Idle, Reservation Length, and NSNP in numeric order. The progress bar still lists all ten modules in their fixed 00–09 numeric order regardless of this click-through order — the same convention used for the Smartalock wizard's 00/02 reorder. Its state lives under `project.floorsense.wizard` in the same `configurationProject` localStorage key described below — `setupPath`, `reservationTypes`, `checkinMethods`, `prebookingAdvance`, `businessHours`, `defaultReservationTimes`, `checkinWindow`, `idleRelease`, `reservationLength` and `nsnp` respectively. Module 09 is currently the last one built, so its next-arrow is disabled.
